...
find ../data/admixture/ -name "*.bim" | grep -e "pruned" > ../data/admixture/PLINK_merged/merge_pruned.list.temp ;#may be an error here, not sure where the bim files are, use right dir
sed 's/.bim//g' ../data/admixture/PLINK_merged/merge.list.temp > ../data/admixture/PLINK_merged/merge_pruned.list ;
rm ../data/admixture/PLINK_merged/merge_pruned.list.temp ;

#Merge all projects into a single PLINK fileset
plink --merge-list ../data/admixture/PLINK_merged/merge_pruned.list --out ../data/admixture/PLINK_merged/merge_pruned
...
admixture ../PLINK_merged/merge_pruned.bed 3 -j16 #16 ncpus in theory; 3 ancestral pops
...
for chr in {1..22}; do
    rm -f ../data/gen_maps/rfmix2_altered/chr${chr}.b38.alt.gmap
