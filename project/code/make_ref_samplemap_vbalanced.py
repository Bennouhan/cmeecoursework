...
try:
    Path('../data/sample_maps/sample_map_no_admix_vbalanced.txt').unlink()
except FileNotFoundError:
    pass

new_map = open('../data/sample_maps/sample_map_no_admix_vbalanced.txt', "w+")
...
### Randomly samples from subpopulations larger than 6, adding all but 6 to a list which will be excluded when writing the new file
...
for pop in eur_pop_set: # Europeans
    ...
    if len(temp_ID_ls) > 6:
        temp_ID_ls = sample(temp_ID_ls, len(temp_ID_ls)-6)
...
for tup in sample_tup_ls:
    if tup[0] in set(exclusion_ls):
        ...
        print(tup[0] + "\t" + tup[1] + "\t" + pop, file=new_map)
