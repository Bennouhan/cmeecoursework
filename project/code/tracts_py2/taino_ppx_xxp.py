...
import os
os.chdir("/home/bennouhan/cmeecoursework/project/code/python2_test")
import sys
import scipy
...
usage = "python taino_ppx_xxp.py [subpop] 1 to run without bootstrap \n"\
        "python taino_ppx_xxp.py [subpop] A [B] if no argument provided, runs "\
...
if len(args) == 1:
    print "Not enough arguments, see instructions .txt file or this .py file"
elif len(args) == 2:
    runboots = range(100) # run all instances
elif len(args) == 3:
    runboots = range(int(args[2])) # run just the specified number
elif len(args) == 4:
