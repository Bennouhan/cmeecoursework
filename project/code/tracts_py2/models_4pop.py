...
def ppxx_xxpp(*params):
    ...
    # model
    prop2 = 1 - prop1
    mig[-1, :] = numpy.array([prop1, prop2, 0, 0])

    penultimate_prop1 = prop1 * timefrac
    penultimate_prop2 = prop2 * timefrac
    mig[-2, :] = numpy.array([penultimate_prop1, penultimate_prop2, 0, 0])
...
def ppxx_xxpp_fix(params, fracs):
    ...
    def fun(params ):
        ...
        return propfrommig(ppxx_xxpp((prop1, tstart, prop3, prop4, t3)))[0:n_pops-1] - fracs[0:n_pops-1]
    ...
    # parameter values.
    (prop1, prop3, prop4) = scipy.optimize.fsolve(fun, (.2, .2, .2))
...
def outofbounds_ppxx_xxpp_fix(params, fracs):
    ...
    def fun(params):
        (prop1, prop3, prop4) = params
        return propfrommig(ppxx_xxpp((prop1, tstart, prop3, prop4, t3)))[0:n_pops-1] - fracs[0:n_pops-1]
    (prop1, prop3, prop4) = scipy.optimize.fsolve(fun, (.2, .2, .2)) #(.2,.2) is just the starting point for the
