...
if __name__ == "__main__":
    ...
    for s in ['bins', 'dat']:
        paths[s] = os.path.join(input_dir, common_name + '_' + s)


    paths['preds'] = [
            os.path.join(input_dir, name + '_pred')
            for name in names]
    ...
    p = os.path.join(output_dir, "%s_plot.%s" % (name, plot_format))

    if not overwrite_plot: # if we care about preserving existing plots
        i = 1
        while os.path.exists(p):
            p = os.path.join(output_dir, "%s_plot.%d.%s" % (name, i, plot_format))
            i += 1
    else:
        if os.path.exists(p):
