...
### For each chromosome...
for (chr in 22:22){ #change range as appropriate
  ### Reads fb files
  fb <- fread(paste0('RFMix2/output/RFMix2_output_chr',chr,'.fb.tsv'),nThread=6)
  range <- 1:30#nrow(fb) #when not testing
  ...
  for (subpop in subpops){
    ...
    output <- mclapply(range, function(x)
                       sum_pop_genotypes(get(subpop), x), mc.cores=6)
